####################################################################################
#  Figure S7: Non-facility birth and EPEs in 2015 - country-specific coefficients  #
####################################################################################
...
# 6. Align and resample rasters to a common (1km) resolution.
...
# 9. Prepare data for mapping: absolute numbers (Figure S7a) - mask and resample to 20x20 km.
# 10. Prepare data for mapping: per 1,000 live births (Figure S7b) - mask and resample to 20x20 km.
# 11. Prepare GADM data for mapping and visualization.
# 12. Generate and save maps for Figure S7(a) and S7(b), and create combined figures.
####################################################################################
...
####  Notes: This is equation (2) and (4) in the supplementary appendix        #####
####  Notes: We use country-specific coefficients here                         #####
####################################################################################
...
####################################################################################
####  Step 9: Prepare Data for Mapping (Figure S7(a): Absolute Numbers)        #####
...
####################################################################################
####  Step 10: Prepare Data for Mapping (Figure S7(b): Per 1'000 Live Births)   #####
####  Notes: Mask results to the 21 SSA countries and resample to 20km X 20km  #####
####################################################################################
...
####################################################################################
####  Step 12: Generate and Save Maps for Figure S7(a) and S7(b)               #####
####################################################################################

####  Load the attribution data for mapping of Figure S7(a) and S7(b) 
# Load the attribution raster for Figure S7(a) 
...
# Load the attribution raster for Figure S7(b) 
...
epe_days_exposure_country_risk_align_birth_2015_sum_filtered_20km[epe_days_exposure_country_risk_align_birth_2015_sum_filtered_20km == 0] <- NA
epe_days_exposure_country_risk_align_birth_2015_sum_filtered_20km_per_thsd_filtered[epe_days_exposure_country_risk_align_birth_2015_sum_filtered_20km_per_thsd_filtered == 0] <- NA
...
# Create Map S7(a) - Number of non-facility births
map_a <- ggplot() +
...
# Define colors based on the QGIS symbology
colors_b <- c(
  "#64abb0",  # <= -6.5
  "#66adb0",  # -6.5 - -5.5
  "#6db2af",  # -5.5 - -4.5
...
# Create Map S7(b) - Per thousand rate
map_b <- ggplot() +
...
ggsave("output/Figures/attribution_plot_heterogeneity_absolute.pdf", 
       map_a, 
       width = 8, height = 8, dpi = 300)

ggsave("output/Figures/attribution_plot_heterogeneity_per1000.pdf", 
       map_b, 
       width = 8, height = 8, dpi = 300)
...
ggsave("output/Figures/attribution_plot_combined_heterogeneity.pdf",
       combined_map,
