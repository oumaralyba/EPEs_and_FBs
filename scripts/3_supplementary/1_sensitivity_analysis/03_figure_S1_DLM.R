################################################################
########     Figure S1 : Distributed lag linear model   ########
...
get_model_data <- function(model, terms) {
  coef <- coef(model)
...
plot_data <- get_model_data(ols_fe_dhs_cbd_85, terms_to_plot)
...
ggsave("output/Figures/Figure_prec_85_count_DLM.pdf",width=15,height=8)
