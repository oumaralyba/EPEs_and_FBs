################################################################
#### Table 1 : Effects of EPEs on Facility-based birth      ####
################################################################
...
# Lets compute the decrease in percentage of the sample mean of Facility-based birth after an additional day with EPE for our preferred model in column (4)
mean_value <- final_data %>%
  ...
  pull(weighted_mean)
(ols_fe_dhs_cbd[["coefficients"]][["prec_85_count_3d_0_lag"]]/mean_value)*100

# Lets compute the confidence interval of the decrease in percentage of the sample mean of Facility-based birth 
coef_int <-  confint(ols_fe_dhs_cbd, parm="prec_85_count_3d_0_lag", level = 0.95, se = "cluster") 
(coef_int$`2.5 %`/mean_value)*100
(coef_int$`97.5 %`/mean_value)*100
...
etable(model_list,
       ...
       dict = c("prec_85_count_3d_0_lag" = "N of days over 85 percentiles",
                ...
                "Facility_based_births*1000" = "Facility-based birth (per 1,000 births)"),  # Labels for variables
       fitstat = c("n","r2"),
       extralines = list(
       ...
       file = paste(output_path , "Tables/Table_1_prec_85_count_3d_0_lag.tex", sep = ""),
       replace = TRUE)
