...
get_model_data <- function(model, model_name) {
...
plot_data <- do.call(rbind, lapply(names(models), function(mn) get_model_data(models[[mn]], mn)))
...
ggplot(plot_data, aes(x = Term_label, y = Estimate, ymin = Conf.Low, ymax = Conf.High, color = Model)) +
  geom_pointrange() +
  # Add percentage over each coefficient
  geom_text(aes(y = LabelPos, label = sprintf("%.1f%%", PercentageChange_trunc)), vjust = 0, hjust = 0.5, color = "black", size = 4) +
  # geom_text(aes(label =  PercentageChange_trunc), vjust = -0.5, hjust = 0.5, color = "black") +
  # Add vertical dashed lines for each group
  ...
  theme(
    ...
    axis.line.x = element_line(color = "black", size = 0.5),  # Add black line on the x-axis
    axis.line.y = element_line(color = "black", size = 0.5)   # Add black line on the y-axis
  ) +
...
# Print the plot
ggsave("output/Figures/Heterogeneity_EPE.pdf", width = 18, height = 7) # Adjust size as needed
