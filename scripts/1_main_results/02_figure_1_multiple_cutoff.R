################################################################
#### Figure 1 : Effects of EPEs on Facility-based birth     ####
################################################################
...
ggplot(combined, aes(x = cut_off, y = estimate, color = as.factor(time_window))) +
  ...
  scale_y_continuous(breaks = seq(-25, 20, 5), labels = scales::comma,    sec.axis = sec_axis(~ . * 100 / sample_mean, name = "Decrease from baseline (%)")
  )  +
scale_color_viridis_d(name = "Time window", labels = function(x) paste0(x, " days"), option = "turbo") +
  ...
  theme(
    ...
    # panel.grid.major = element_line(linewidth = 0.05, linetype = 'dashed',colour = "black")
  ) +
...
ggsave("output/Figures/ols_fe_dhs_cbd_n_cutoff_nd_0_lag_combined.pdf", width=16,height=8, bg = "transparent")
...
ggplot(combined %>% filter(time_window==3), aes(x = cut_off, y = estimate)) +
  ...
  geom_point(position = position_dodge(width = 0.8), color = viridis(1, option = "turbo", begin = 0),  size = 1.25) +
  ##### New
  scale_y_continuous(breaks = seq(-25, 20, 5), labels = scales::comma,    sec.axis = sec_axis(~ . * 100 / sample_mean, name = "Decrease from baseline (%)")
  )  +
  ######
...
ggsave("output/Figures/ols_fe_dhs_cbd_n_cutoff.pdf",width=16,height=8)
