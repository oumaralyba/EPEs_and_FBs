####################################################################################
####     Figure 3 & S5 : Non-facility birth and EPEs in 2015 (by country)      #####
####################################################################################
...
# 6. Aggregate results to annual and 20x20 km resolution for mapping.
# 7. Prepare GADM country boundaries to mask results to study countries.
# 8. Prepare data for mapping (Figure 3a: Absolute Numbers).
# 9. Prepare data for mapping (Figure 3a: Per 1'000 live births).
...
####################################################################################
####  Step 6: Aggregate Results to Annual and 20x20 km Resolution              #####
...
epe_days_exposure_align_birth_2015_sum_filtered_20km[epe_days_exposure_align_birth_2015_sum_filtered_20km == 0] <- NA
epe_days_exposure_align_birth_2015_sum_20km_per_thsd_filtered[epe_days_exposure_align_birth_2015_sum_20km_per_thsd_filtered == 0] <- NA
...
epe_days_exposure_align_birth_2015_sum_filtered_20km$bin <- cut(
  ...
  right = TRUE           # intervals like [0,1], (1,5], …
)
...
ggsave("output/Figures/attribution_plot_absolute.pdf", 
       map_a, 
       width = 8, height = 8, dpi = 300)

ggsave("output/Figures/attribution_plot_per1000.pdf", 
       map_b, 
       width = 8, height = 8, dpi = 300)
...
ggsave("output/Figures/attribution_plot_combined.pdf",
       combined_map,
...
####################################################################################
####  Step 12: Calculate Non-facility birth due to EPE by Country (Figure S5)  #####
####################################################################################
...
# Save the plot
ggsave("output/Figures/HB_by_country_figure.pdf", 
       width = 15, height = 10)
...
global(epe_days_exposure_align_birth_2015_sum_filtered, fun = 'sum', na.rm = TRUE) #  29165.23

# Sum of Non-facility birth due to EPE (conf low) in the countries of interest
global(epe_days_exposure_conf_low_align_birth_2015_sum, fun = 'sum', na.rm = TRUE) # 76634.42

# Sum of Non-facility birth due to EPE (conf high) in the countries of interest
global(epe_days_exposure_conf_high_align_birth_2015_sum, fun = 'sum', na.rm = TRUE) # # 13405.89
